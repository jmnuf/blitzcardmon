...
const Home: NextPage = () => {
	...
	const lang = cardLanguages ? cardLanguages[langIndex] ?? "mandarin" : "mandarin";
	...
	return (
		<>
			<Head>
				...
				<meta name="description" content="Card learner created with create-t3-app" />
			...
			<main className="min-h-screen w-full bg-gradient-to-b from-purple-800 to-violet-900 flex flex-col items-center justify-center p-4">
				<div className="contatiner mx-auto flex flex-col items-center justify-center text-purple-400">
					...
					<div className="flex flex-col w-full items-center justify-center pt-6 text-2xl">
						{hello.data ? <p>{hello.data.greeting}</p> : <p>Loading salute..</p>}
						{cards.data ? <p>Cards found for {`"${lang}"`}: {cards.data.count}</p> : <p>Loading {`"${lang}"`} cards count...</p>}
					</div>
				</div>
			</main>
		</>
